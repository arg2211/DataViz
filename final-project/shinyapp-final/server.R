...
tweets.df <- read.csv("./shinyapp-final/t2.csv", header = TRUE)
...
tweets.df$hillary = grepl('hillary|clinton', tweets.df$text)
tweets.df$bernie = grepl('bernie|sanders', tweets.df$text)
tweets.df$ted = grepl('ted|cruz', tweets.df$text)
tweets.df$donald = grepl('donald|trump', tweets.df$text)

# subset(tweets.df, subset=tweets.df$hillary)
